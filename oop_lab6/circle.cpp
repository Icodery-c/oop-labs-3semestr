...
QRect Circle::getBoundsAfterResize(int dw, int dh) const
{
    int newRadius = m_radius + dw; // Используем только dw для круга
    if (newRadius < 5) newRadius = 5;

    QPoint topLeft(m_position.x() - newRadius, m_position.y() - newRadius);
    return QRect(topLeft, QSize(newRadius * 2, newRadius * 2));
}

void Circle::setRadius(int radius, const QRect& bounds)
{
    if (radius > 0) {
        // Проверяем, не выйдет ли круг за границы при новом размере
        int delta = radius - m_radius;
        if (canResize(delta, delta, bounds)) {
            m_radius = radius;
            qDebug() << "Circle radius changed to:" << radius;
        } else {
            qDebug() << "Circle cannot resize to" << radius << "- out of bounds";
        }
    }
}
