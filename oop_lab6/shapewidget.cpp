...
void ShapeWidget::paintEvent(QPaintEvent *event)
{
    ...
    // Подсчитываем выделенные фигуры
    int selectedCount = 0;
    for (int i = 0; i < m_storage.getCount(); ++i) {
        if (m_storage.getShape(i)->isSelected()) {
            selectedCount++;
        }
    }
    ...
    // Дополнительная информация о пересечениях
    if (m_currentTool == Tool::Select) {
        painter.drawText(10, 40, "ЛКМ - выделить все под курсором | Ctrl+ЛКМ - добавить к выделению");
    }
}
void ShapeWidget::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        if (m_currentTool == Tool::Select) {
            // Режим выделения - выделяем ВСЕ фигуры под курсором
            bool ctrlPressed = event->modifiers() & Qt::ControlModifier;
            m_storage.selectShapesAt(event->pos(), ctrlPressed);
            emit selectionChanged();
        } else {
...
void ShapeWidget::keyPressEvent(QKeyEvent *event)
{
    bool handled = true;
    int step = 5;
    QRect bounds = rect(); // Границы - весь виджет
    ...
    case Qt::Key_Left:
        m_storage.moveSelected(-step, 0, bounds);
        update();
        break;
    case Qt::Key_Right:
        m_storage.moveSelected(step, 0, bounds);
        ...
        break;
    case Qt::Key_Up:
        m_storage.moveSelected(0, -step, bounds);
        update();
        break;
    case Qt::Key_Down:
        m_storage.moveSelected(0, step, bounds);
    ...
    case Qt::Key_Equal:
        if (event->modifiers() & Qt::ControlModifier) {
            m_storage.resizeSelected(5, 5, bounds);
            update();
        }
        break;
    case Qt::Key_Minus:
